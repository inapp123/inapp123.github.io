...
```
有三台虚拟机，分别命名为server,reverse_proxy,Artoria
server,就是处于内网中的服务器。
reverse_proxy,就是中转服务器。
Artoria大家都懂。。。客户机。
...
如果你的主机与中转服务器架构，系统，位数都相同的话，那么你可以直接把编译出来的客户端传到服务器上使用。请执行以下命令，并跳至下方"4.配置lunnel服务端"
...
```
若你的服务器的系统为windows，请填空windows
若你的服务器的系统为linux，请填空linux
若你的服务器的系统为Mac os(OSX)，请填空drawin
```

将<请填空2>按照以下规则替换：

```
若你的服务器cpu是龙芯，请务必让我膜拜一下并填空mips64le
若你的服务器为32位，请填空386
若你的服务器为64位，请填空amd64
```
...
```
将<请填空1>替换为服务器的域名。如果没有域名的话请把整行删去。
将<请填空2>替换为服务器的连接端口(＞1，＜65535，尽量大一些。）
将<请填空3>替换为服务器的http(网页)端口(＞1，＜65535，尽量大一些。）
将<请填空4>替换为服务器的https(加密网页)端口(＞1，＜65535，尽量大一些。）
...
第一种方式适用于你没事想把它捞出来看看长啥样了，而第二种方式适用于你就想让他跑，并不想管他。
...
```bash
scp <中转服务器用户名>@<中转服务器ip>:~/client.tar.gz client.tar.gz
```
...
```
将<请填空1>替换为中转服务器的ip
将<请填空2>替换为中转服务器的连接端口(上面写过的那个)
将<请填空3>替换为想让外网连接的端口,minecraft默认为25565
将<请填空4>替换为本地游戏服务器的端口,minecraft默认为25565
将<请填空5>替换为上面记下来的密码
```
...
接着把它用上方提到的方式丢到后台，启动游戏服务器，跳转到Chapter.3吧!



### 2.若你的服务器是Windows系统:

先在服务器上下载pscp:https://the.earth.li/~sgtatham/putty/latest/w32/pscp.exe  将其复制到你想安装lunnel的位置，然后按住shift,点击右键，选择“在此处打开命令窗口”
...
```
pscp <中转服务器用户名>@<中转服务器ip>:/home/<中转服务器用户名>/client.tar.gz client.tar.gz
```
...
解决方案：服务端使用1024以下的端口时需root权限。。。可以在启动命令前加sudo,或者将端口改为大于1024的值。
